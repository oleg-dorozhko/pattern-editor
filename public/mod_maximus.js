//!() !
console.log('mod maximus on');
var PNG = require('pngjs').PNG;
module.exports.maximus = maximus;
...
function maximus_compareColors(color,color2)
{
	if(
			(color2[0]==color[0]) && 
			(color2[1]==color[1]) && 
...
{
	var colors2 = JSON.parse(JSON.stringify(colors));
	...
	for(var i=0;i<colors.length;i++)
	{
		...
		{
			if(maximus_compareColors(colors[i],colors2[j])==true) count++;
		}
		if(count>maxcount)
		{
			maxcount=count;
...
function maximus(im)
{
	...
	for (var y = 0; y < h; y+=3) {
		...
		for (var x = 0; x < w; x+=3) {
				...
				var points_arround	= getWHDNeighbors(x, y, 1, 1,w,h);
				var max_color = getMaxColor(getColorsForPoints(im,points_arround));
				
				var x1=x/3;
				var y1=y/3;
				var ind = w2*y1+x1 << 2;
				
				newpng = fillRectangleFast(newpng, x1, y1, 1, 1, max_color );
				// newpng.data[ind]=avg_color[0];
				// newpng.data[ind+1]=avg_color[1];
				// newpng.data[ind+2]=avg_color[2];
				// newpng.data[ind+3]=255;
